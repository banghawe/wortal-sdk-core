...
/**
 * Poki implementation of the Context API.
 * @hidden
 */
export class ContextPoki extends ContextBase {
    ...
    protected shareLinkAsyncImpl(payload: LinkSharePayload): Promise<string | void> {
        return new Promise((resolve, reject) => {
            Wortal._internalPlatformSDK.shareableURL(payload.data)
                .then((url: string) => {
                    resolve(url);
                })
                .catch((error: any) => {
                    //TODO: test and throw correct error
                    reject(error);
                });
        });
    }
