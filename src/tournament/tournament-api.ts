...
/**
 * The Tournament API provides a way for developers to create and manage tournaments for their game.
 * Tournaments are surfaced in-game to players, and players can join and compete in tournaments with their friends.
 * Tournaments are created with a specific context, and players can only join a tournament if they are in the same context.
 * For example, a game can create a tournament for a specific level, and only players who are currently playing that level
 * can join the tournament. Tournaments can be created for any context, and the context can be changed at any time.
 * @module tournament
 */
export class TournamentAPI {
    private _tournament: TournamentBase;

    constructor(impl: TournamentBase) {
        this._tournament = impl;
    }
    ...
    public createAsync(payload: CreateTournamentPayload): Promise<Tournament> {
        return this._tournament.createAsync(payload);
    }
    ...
    public getAllAsync(): Promise<Tournament[]> {
        return this._tournament.getAllAsync();
    }
    ...
    public getCurrentAsync(): Promise<Tournament> {
        return this._tournament.getCurrentAsync();
    }
    ...
    public joinAsync(tournamentID: string): Promise<void> {
        return this._tournament.joinAsync(tournamentID);
    }
    ...
    public postScoreAsync(score: number): Promise<void> {
        return this._tournament.postScoreAsync(score);
    }
    ...
    public shareAsync(payload: ShareTournamentPayload): Promise<void> {
        return this._tournament.shareAsync(payload);
    }
