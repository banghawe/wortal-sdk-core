 ...
 */
export type ContextFilterViber = 'NEW_CONTEXT_ONLY'
    | 'INCLUDE_EXISTING_CHALLENGES'
    | 'NEW_PLAYERS_ONLY'
    | 'NEW_INVITATIONS_ONLY';

export interface ContextChoosePayloadViber {
    /**
...
export interface ContextSizeResponseViber {
...
/** @hidden */
export interface CurrentContextViber {
    id: string | null;
    type: 'SOLO' | 'THREAD';
    size: number;
    connectedPlayers: ContextPlayerViber[];
}

/** @hidden */
export interface InitializeResponseContextViber {
    id: string | null;
    type: 'SOLO' | 'THREAD';
    size: number;
